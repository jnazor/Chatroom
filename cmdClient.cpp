/*
 * Command Client program being used by Jolie Nazor and Kristi Yost for the CS 450 Chatroom 
...
int main(int argc, char * argv[]) {
	...
	while (fgets(buf, sizeof(buf), stdin)) {
		...
		//cout << "Before tolist if statement." << endl;
		if(tolist) {
			//cout << "Are we here?" << endl;
			short size;
			readn(s, (char *) &size, sizeof(short));
			for(short i = 0; i < size; i++) {
				char name[MAX_LINE];
			        short namelen = 0;
				readn(s, (char *) &namelen, sizeof(short));
				namelen = ntohs(namelen);
				//cout << "namelen: " << namelen << endl;
				readn(s, name, namelen);
...
{         
    ...
    no_left = size;
    while (no_left > 0) { 
	//cout << "no_left: " << no_left << endl;
        no_read = recv(sd,ptr,no_left, 0);
	//cout << "no_read: " << no_read << endl;
	//cout << "Size of ptr: " << strlen(ptr) << endl;
        if(no_read <0)  
		return(no_read);
        if (no_read == 0) 
		break;
